...
static py::tuple candidate_to_tuple(const CandidateExport& c) {
	...
	return py::make_tuple(c.pool_score, c.final_score, c.reduction, k, l, c.basis_dim, c.bucket_size, tD, c.num_better_dim, c.num_better_red, c.num_better_pool_score);
}
...
PYBIND11_MODULE(gf2lib, m) {
	...
	py::class_<Matrix>(m, "Matrix")
		...
		.def(
			"seed",
			[](const Matrix& mat, std::uint32_t base_seed, std::uint32_t step) { return matrix_seed(mat, base_seed, step); },
			py::arg("base_seed") = 0U,
			py::arg("add_seed") = 0U)
	...
	py::class_<PyTohpeGenerator>(m, "TohpeGenerator")
		...
		.def("make2", &PyTohpeGenerator::make2, py::arg("row1"), py::arg("row2"), py::return_value_policy::move, py::keep_alive<0, 1>())
	...
	py::class_<PyToddGenerator>(m, "ToddGenerator")
		...
		.def("make2", &PyToddGenerator::make2, py::arg("row1"), py::arg("row2"), py::return_value_policy::move, py::keep_alive<0, 1>())
    ...
    py::class_<CandidateExport>(m, "CandidateExport")
        ...
        .def("__repr__", [](const CandidateExport &c) {
            return "CandidateExport(score=" + std::to_string(c.pool_score) +
                   ", reduction=" + std::to_string((long long)c.reduction) +
                   ", k=" + std::to_string((long long)c.k) +
                   ", l=" + std::to_string((long long)c.l) +
                   ", basis_dim=" + std::to_string((long long)c.basis_dim) +
	...
	py::class_<Stats>(m, "Stats")
		...
		.def("__repr__", [](const Stats &s) {
			return "Stats(total=" + std::to_string((unsigned long long)s.total) +
				...
				", mean_score=" + std::to_string(s.mean_score) +
				", max_score=" + std::to_string(s.max_pool_score) +
	...
	py::class_<ExplorationScore>(m, "ExplorationScore")
        ...
        .def("__repr__", [](const ExplorationScore &r) {
		...
		.def("__str__", [](const ExplorationScore &r) {
            return "(" + std::to_string(r.wdim) +
                   ", " + std::to_string(r.wbucket) +
                   ", " + std::to_string(r.wred) +
				   ", " + std::to_string(r.wvw) +
                   ")";
        })
		.def("__len__", [](const ExplorationScore &r) {
			// Check if wvw has been set (you need a way to detect this)
			// For simplicity, always return 4 or add a member variable to track
			return 4;
		})
		.def("__getitem__", [](const ExplorationScore &r, py::ssize_t index) {
			if (index < 0 || index >= 4) {
		...
		.def(py::pickle(
			[](const ExplorationScore &r) {
				return py::make_tuple(r.wred, r.wdim, r.wbucket, r.wvw);
			},
			[](py::tuple t) {
	...
	py::class_<FinalizationScore>(m, "FinalizationScore")
        ...
        .def("__repr__", [](const FinalizationScore &r) {
		...
		.def("__str__", [](const FinalizationScore &r) {
            return "(" + std::to_string(r.wdim) +
                   ", " + std::to_string(r.wbucket) +
                   ", " + std::to_string(r.wred) +
                   ", " + std::to_string(r.wtohpe_dim) +
                   ", " + std::to_string(r.wvw) +
                   ")";
        })
		.def("__len__", [](const FinalizationScore &r) {
			return 5;
		})
		.def("__getitem__", [](const FinalizationScore &r, py::ssize_t index) {
			if (index < 0 || index >= 5) {
		...
		.def(py::pickle(
			[](const FinalizationScore &r) {
				return py::make_tuple(r.wred, r.wdim, r.wbucket, r.wtohpe_dim, r.wvw);
			},
			[](py::tuple t) {
    ...
    py::class_<Result>(m, "Result")
        ...
        .def("__repr__", [](const Result &r) {
            return "Result(chosen=" + std::to_string(r.chosen.size()) +
                   ", states=" + std::to_string(r.states.size()) +
    ...
    py::class_<PolicyConfig>(m, "PolicyConfig")
        ...
        .def("__repr__", [](const PolicyConfig &c) {
            return "PolicyConfig(num_samples=" + std::to_string((long long)c.num_samples) +
                   ...
                   ", top_pool=" + std::to_string((long long)c.top_pool) +
	...
	m.def(
		...
		[](
			...
			return policy_iteration_impl(data,
										 pcfg, seed, add_seed);
		},
		py::arg("cur_mat"),
		py::arg("policy_cfg")=PolicyConfig{},
		py::arg("seed")=21,
		py::arg("add_seed")=0
	);
}
