...
// Knobs whose value varies with the choice of z while y is held fixed. An
// exploration expression touching any of these cannot be reduced to
// "rank by reduction".
const KnobMask& z_varying_knobs() {
    static const KnobMask mask = make_mask({Knob::bucket, Knob::nbucket, Knob::zw, Knob::nzw, Knob::zsize,
                                            Knob::bucket_id, Knob::max_red, Knob::nmax_red});
    return mask;
}
...
// Abstract interpretation deciding whether the program is a non-decreasing
...
bool PolicyProgram::compute_ranks_by_reduction_() const {
    ...
    struct Value {
        bool  affine_in_red = false; // slope is meaningful only when true
        float slope         = 0.0f;
    ...
    for (const Instr& in : code_) {
        ...
        case Op::LoadKnob: {
            ...
            if (knob == Knob::red) {
                ...
                v.slope         = 1.0f;
            } else if (knob == Knob::nred) {
                // red / bn / 2 -- bn is fixed within an iteration and positive,
                // so this stays a positive multiple of red. The exact slope does
                // not matter, only its sign.
                v.affine_in_red = true;
                v.slope         = 1.0f;
            } else {
                // Constant for a fixed y (dim, yw and the normalizers), but the
        ...
        case Op::Sub: {
            ...
            if (v.constant)
                v.value = in.op == Op::Add ? a.value + b.value : a.value - b.value;
            if (!push(v))
                return false;
            break;
        }
        case Op::Mul: {
            ...
            if (a.constant && b.constant) {
                ...
                // red multiplied by an unknown quantity: sign is not decidable.
                return false;
            }
            if (!push(v))
                return false;
        ...
        case Op::Div: {
            ...
            if (!a.affine_in_red || !b.affine_in_red)
                return false;
            if (b.slope != 0.0f)
                return false; // red in a denominator is not affine
            Value v{};
            if (b.constant && b.value != 0.0f) {
                v.affine_in_red = true;
                v.slope         = a.slope / b.value;
                v.constant      = a.constant;
                if (v.constant)
                    v.value = a.value / b.value;
            } else if (a.slope == 0.0f) {
                v.affine_in_red = true;
                v.slope         = 0.0f;
            } else {
        ...
        default: {
            // Any other operation (transcendental, comparison, select, min/max)
            // is only safe when its inputs do not depend on red -- then the
            // result is a fixed offset. Otherwise the ordering is not provably
            // monotone and the fast path must be given up.
            const auto [pops, pushes] = op_stack_effect(in.op);
            if (sp < static_cast<std::size_t>(pops))
                return false;
            bool red_free = true;
            for (int i = 0; i < pops; ++i) {
                const Value& v = stack[sp - 1 - static_cast<std::size_t>(i)];
                if (!v.affine_in_red || v.slope != 0.0f)
                    red_free = false;
            }
            sp -= static_cast<std::size_t>(pops);
            if (!red_free)
                return false;
            Value v{};
            v.affine_in_red = true;
    ...
    // A strictly positive slope reproduces reduction-descending order. A zero
    // slope makes every candidate tie, which the reduction path would break
    // differently, so it does not qualify.
    return result.affine_in_red && result.slope > 0.0f;
}
