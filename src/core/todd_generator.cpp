...
namespace gf2 {
	...
	class TopKPool {
	public:
		explicit TopKPool(std::size_t limit, Score keyfn = Score{})
			: limit_(limit), keyfn_(std::move(keyfn)) {}
	...
	private:
		...
		static bool worse_node_(Node const& a, Node const& b) noexcept {
			...
			if (a.cand.reduction != b.cand.reduction) return a.cand.reduction > b.cand.reduction;
			if (a.cand.k != b.cand.k) return a.cand.k < b.cand.k;
			if (a.cand.l != b.cand.l) return a.cand.l < b.cand.l;
	...
	std::vector<Candidate> pick_softmax_view(TopKPool<FinalizationScore>& pool, std::size_t n, float temperature, PyRNG& rng) {
		auto all = pool.release_unsorted();
		if (all.empty() || n == 0)
			return {};
		if (n > all.size())
	...
	Result build_result(TopKPool<FinalizationScore>& pool,
						bool is_best,
						std::size_t num_candidates,
						float temperature,
						PyRNG& pick_rng,
						SeenValues const& svs,
						std::shared_ptr<MatrixWithData> data,
						Stats const& global_stats,
						std::uint64_t base_seed) {
		auto chosen_view = is_best ? pick_best_view(pool, num_candidates) : pick_softmax_view(pool, num_candidates, temperature, pick_rng);
	...
	auto policy_iteration_impl(const std::shared_ptr<MatrixWithData>& data, PolicyConfig config, index_t seed, index_t add_seed) -> Result {
		...
		Stats global_stats;
		{
			...
			if (max_tohpe > 0) {
				...
				auto accept_anyway = [&](auto red) { return red <= 0 && non_improving_prob && local_rng.rand_double(0.0, 1.0) < non_improving_prob; };
				...
				for (auto const& coefs : coefs_list) {
					...
					for(auto [z, red]: gen.best_z_n(vec, tohpe_sample)){
						...
						Candidate c(0, (Int)red, k_single_sentinel<Int>(), k_single_sentinel<Int>(), vec, (Int)dim, (Int)bucket_size, nsptr);
			...
			if (!try_only_tohpe || pool.size() < min_pool) {
				...
				#endif
				{
					...
					for (std::size_t i = 0; (i < buckets.size()) ; ++i) {
						if (i >= max_z_to_research && (local_pool.size() >= min_pool || local_beyond_pool.size() >= min_pool)) {
						...
						auto ns = std::make_shared<NullSpace>((l == k_single_sentinel<decltype(l)>()) ? local_gen.make(k) : local_gen.make(k, l));
						...
						auto coefs_list = local_rng.sample_small_unique_bitvectors(dim, num_samples);
						auto coefs_list = (len > 1) ? local_rng.sample_small_unique_bitvectors(dim, num_samples): local_rng.sample_special_bitvec(ns->basis(), ptr->a, ptr->b, num_samples);
						...
						for (auto& coefs : coefs_list) {
							...
							auto [score, tohpe] = (beyond(red)) ? local_beyond_pool.push(std::move(c)): local_local_pool.push(std::move(c));
					...
					#endif
					{
						...
						global_stats.mean_reduction = (global_stats.mean_reduction * gnz + stats.mean_reduction * lnz) / (gnz + lnz);
	...
	void SeenValues::merge_from(const SeenValues& other) {
		...
		pool_scores_sorted.insert(pool_scores_sorted.end(), other.pool_scores_sorted.begin(), other.pool_scores_sorted.end());
