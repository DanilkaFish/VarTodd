...
namespace gf2 {
	...
	bool RowEq::operator()(RowCView a, RowCView b) const noexcept {
		return a.size() == b.size() && a.blocks() == b.blocks() && equal_row_blocks(a.data(), b.data(), a.blocks(), a.size());
	}
	...
	Matrix::Matrix(index_t rows, index_t cols) : rows_(rows), cols_(cols), blocks_per_row_(ceil_div64(cols)), data_(rows * blocks_per_row_, 0ULL) {}
	...
	RowCView Matrix::operator[](index_t i) const noexcept { return RowCView(&data_[i * blocks_per_row_], cols_, blocks_per_row_); }
	...
	bool Matrix::operator==(const Matrix& rhs) const noexcept { return rows_ == rhs.rows_ && cols_ == rhs.cols_ && data_ == rhs.data_; }

	Tensor3D::Tensor3D(index_t n) : n_{n}, t_(static_cast<std::size_t>(n) * static_cast<std::size_t>(n) * static_cast<std::size_t>(n)) {}

	// Tensor3D::Tensor3D(index_t n, Row data) : n_{n}, t_(std::move(data)) {
	// 	const std::size_t need = static_cast<std::size_t>(n) * static_cast<std::size_t>(n) * static_cast<std::size_t>(n);
	// 	if (t_.size() != need)
	// 		throw std::invalid_argument("Tensor3D: data size mismatch");
	// }
	...
	index_t Tensor3D::lin(index_t i, index_t j, index_t k) const noexcept {
		...
		return static_cast<index_t>(static_cast<std::size_t>(i) + N * static_cast<std::size_t>(j) + N * N * static_cast<std::size_t>(k));
	}
