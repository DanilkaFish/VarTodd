...
namespace gf2 {
	...
	template <class Ptr> class BasicRowView {
	  ...
	  public:
		...
		index_t find_first() const noexcept {
			...
			std::size_t k = std::size_t(p - p_);
        	std::size_t bit = std::countr_zero(*p);
        	return (k << 6) + bit;
		}

		index_t find_next(index_t pos) const noexcept {
			if (size_ == 0 || pos >= size_ - 1) return npos;
			++pos;
	...
	// ------------------- OPS --------------------------
	template <WritableRow L, ReadableRow R>
	inline L& xor_assign_avx2(L& lhs, const R& rhs) noexcept {
		assert(lhs.size() == rhs.size());
		assert(lhs.blocks() == rhs.blocks());

		uint64_t* __restrict dst = lhs.data();
		const uint64_t* __restrict src = rhs.data();
		index_t n = lhs.blocks();

		index_t i = 0;

		// 4x uint64 per __m256i
		for (; i + 4 <= n; i += 4) {
	...
	inline void xor_noalias(uint64_t*   __restrict dst,
	...
	template <WritableRow L, ReadableRow R> inline L& operator^=(L& lhs, const R& rhs) noexcept {
		...
		xor_noalias(lhs.data(), rhs.data(), lhs.blocks());
		return lhs;
	}
	...
	template <ReadableRow L, ReadableRow R> inline std::strong_ordering operator<=>(const L& lhs, const R& rhs) noexcept {
		...
		const index_t last = nb - 1;
		// Choose an ordering. This is "block 0 first" (LSB-side first).
		for (index_t k = 0; k < last; ++k) {
			if (a[k] < b[k])
		...
		const uint64_t al = a[last];
		const uint64_t bl = b[last];
	...
	class Row {
	  ...
	  public:
		...
		Row(const T& readable_row) : size_(readable_row.size()), blk_(readable_row.data(), readable_row.data() + readable_row.blocks()) {}
	...
	template <ReadableRow L, ReadableRow R>
		requires requires(const L& l, const R& r) { l.size() == r.size(); }
	inline Row operator^(const L& a, const R& b) {
	...
	// ------------------- MATRIX --------------------------
	struct RowHash {
		using is_transparent = void; // enables find(RowCView) without constructing Row
	...
	class Matrix {
	  public:
		...
		static auto zeros(index_t rows, index_t cols) -> Matrix;
		static auto identity(index_t n) -> Matrix;
		static auto from_rows(const std::vector<Row>& rows) -> Matrix;
		static auto from_npy(const std::string& npy_path) -> Matrix;
		void reset();
