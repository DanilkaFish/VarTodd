...
OUT="${OUT:-$ROOT/benchmark_results/gf_comparison.csv}"
...
if [[ "${APPEND:-0}" == "1" ]]; then
    ...
    OUT="$(mktemp -p "$(dirname -- "$FINAL_OUT")" ".gf_comparison.XXXXXX.csv")"
    printf '%s\n' "$CSV_HEADER" > "$OUT"
fi
...
for dim in "${DIMS_ARR[@]}"; do
    ...
    if [[ ! -f "$matrix" ]]; then
        printf 'missing GF matrix: %s\n' "$matrix" >&2
        exit 1
    fi

    run_with_limit "VarTODD $dim" "$PYTHON_BIN" "$SCRIPT_DIR/vartodd_fasttodd_mimic.py" "$matrix" --csv "$OUT" --time-limit "$TIME_LIMIT"
