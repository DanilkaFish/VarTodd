...
def _w_tanh(z: float, scale: float = 2.5, sharp: float = 1.5) -> float:
...
class Evaluator(BaseEvaluator):
    ...
    def policy_mapping(self):
        split = max(1, int(self.init_rank * 0.82))
        ranks = [split, 0]
        ...
        self.set_final_scores(
            ...
            [
                FinalizationScore(
                    weights=[fwred, wdim, wbucket, 0.0, 0.0, fwtohpe],
                    centers=[1, 0, 0, 0, 0, 0],
                    pow=2,
                ),
                FinalizationScore(
                    weights=[fwred, 0.5 * wdim, 0.5 * wbucket, 0.0, 0.0, 0.0],
                    centers=[1, 0, 0, 0, 0, 0],
                    pow=2,
                ),
            ],
        ...
        self.set_min_z_to_research(ranks, [20 + 60 * budget, 12 + 30 * budget])
        ...
        self.set_gen_part(ranks, [0.18 + 0.18 * budget, 0.10 + 0.10 * budget])
    ...
    def __call__(self, params: Iterable):
        ...
        if self.best_rank < 100000 and first[0] > self.best_rank + 18:
            return float(first[0] + 5.0)
        rest = self.run(params, self.seeds[3:])
        tcounts = first + rest
        return float(np.quantile(tcounts, 0.7) + 0.02 * np.std(tcounts))


def run_pso(fun: Evaluator, iters: int = 9, particles: int = 4) -> np.ndarray:
    dims = len(fun.extract_active())
    bounds = (np.full(dims, -1.3), np.full(dims, 1.3))
    ...
    optimizer = ps.single.GlobalBestPSO(
        ...
        dimensions=dims,
        options={"c1": 0.60, "c2": 0.55, "w": 0.74},
        bounds=bounds,
    )
...
def entrypoint():
    ...
    xopt = run_pso(fun, iters=5, particles=4)
    fun(xopt)

    target = max(1, int(fun.init_rank * 0.82))
    x_active = fun.set_up_new_init(0, rank_thr=target, xopt=xopt)
    if x_active is not None:
        xopt = run_pso(fun, iters=6, particles=4)
        fun(xopt)
    return fun.get_best()
