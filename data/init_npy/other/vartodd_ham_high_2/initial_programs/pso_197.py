...
def _w_tanh(z: float, scale: float = 2.3, sharp: float = 1.5) -> float:
...
class Evaluator(BaseEvaluator):
    ...
    def policy_mapping(self):
        ...
        self.set_final_scores(
            ...
            [
                FinalizationScore(
                    weights=[fwred, 0.5 * wdim, 0.0, 0.0, 0.0, fwtohpe],
                    centers=[1, 0, 0, 0, 0, 0],
                    pow=2,
                ),
                FinalizationScore(
                    weights=[fwred, 0.2 * wdim, 0.0, 0.0, 0.0, 0.0],
                    centers=[1, 0, 0, 0, 0, 0],
                    pow=2,
                ),
            ],
        ...
        self.set_min_z_to_research(ranks, [18 + 48 * budget, 10 + 24 * budget])
        self.set_max_z_to_research(20000)
        self.set_temperature(0.36)
        ...
        self.set_gen_part(ranks, [0.16 + 0.16 * budget, 0.10 + 0.08 * budget])
...
def run_pso(fun: Evaluator, iters: int = 10, particles: int = 4) -> np.ndarray:
    ...
    optimizer = ps.single.GlobalBestPSO(
        ...
        dimensions=dims,
        options={"c1": 0.55, "c2": 0.55, "w": 0.72},
        bounds=bounds,
    )
...
def entrypoint():
    fun = Evaluator(path_name="init", max_depth=165)
