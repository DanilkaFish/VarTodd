...
def _w_tanh(z: float, scale: float = 2.2, sharp: float = 1.5) -> float:
    ...
    def policy_mapping(self):
        ...
        self.set_pool_scores(
            ...
            [
                ...
                ExplorationScore(weights=[wred, wdim, 0.5 * wbucket, 0.0, 0.0], pow=1),
            ],
        )
        self.set_final_scores(
            ...
            [
                FinalizationScore(
                    weights=[fwred, 0.4 * wdim, wbucket, 0.0, 0.0, fwtohpe],
                    centers=[1, 0, 0, 0, 0, 0],
                    pow=2,
                ),
                FinalizationScore(
                    weights=[fwred, 0.2 * wdim, 0.5 * wbucket, 0.0, 0.0, 0.0],
                    centers=[1, 0, 0, 0, 0, 0],
                    pow=2,
                ),
            ],
        ...
        self.set_gen_part(ranks, [0.18 + 0.18 * budget, 0.12 + 0.12 * budget])
        self.set_todd_width(ranks, [width, 1])
        self.set_beamsearch_width(ranks, [beam, 1])

    def __call__(self, params: Iterable):
        rank = self.run(params, self.seeds)[0]
        return float(rank)
...
def run_pso(fun: Evaluator, iters: int = 8, particles: int = 4) -> np.ndarray:
    ...
    dims = len(x)
    bounds = (np.full(dims, -1.5), np.full(dims, 1.5))
